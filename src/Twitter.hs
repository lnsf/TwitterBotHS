...
getUserTweets user = do
  res <- do
    signedReq <-
      ...
      <$> parseRequest "https://api.twitter.com/1.1/statuses/user_timeline.json"
    man <- newManager tlsManagerSettings
    httpLbs signedReq man
  let dc = eitherDecode $ responseBody res
  case dc of
...
getMentions = do
  res <- do
    req <-
      setQueryString [(B8.pack "count", Just (B8.pack "200"))] <$> parseRequest
 ...
 where
  ...
  match n m =
    (isConversation m || isAtMention m)
      && isReplyToMe n m
      && (not . favorited) m
...
postReply i s = do
  res <- do
    req       <- parseRequest "https://api.twitter.com/1.1/statuses/update.json"
    man       <- newManager tlsManagerSettings
    signedReq <- createSignedReq $ urlEncodedBody
