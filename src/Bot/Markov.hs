...
createBlocks ws i = let (_:w2:ws3) = ws
                        ws2 = (w2:ws3)
...
createTweet hs bs = (`execStateT` Nothing)
  $ replicateM_ 100
  $ do
    v <- get
    when (isNothing v)
  ...
  where
    ...
    isMatch bs = let len = length bs
                 in len <= 10 && cost (map getBId bs) < len `div` 2

    cost = sum . map (flip (-) 1 . length) . group

    fmtToSentence xs = let bs2 = map getW2 xs
                           bs3 = map getW3 xs
